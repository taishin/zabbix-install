...
package "zabbix-release" do
  action :install
  source "/tmp/zabbix-release-6-1.noarch.rpm"
...
execute "create-database-user" do
	 ...
	 EOH
   command "createuser -U postgres zabbix"
   not_if code 
end
...
execute "create-schema" do
   command "psql -f /usr/share/doc/zabbix-server-pgsql-2.0.4/create/schema.sql -U zabbix zabbix"
end

execute "create-schema-data" do
   command "psql -f /usr/share/doc/zabbix-server-pgsql-2.0.4/create/data.sql -U zabbix zabbix"
end

execute "create-schema-images" do
   command "psql -f /usr/share/doc/zabbix-server-pgsql-2.0.4/create/images.sql -U zabbix zabbix"
end
